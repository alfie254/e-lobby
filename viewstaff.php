...
<form id ="search" method="post">
 ...
 <?php
  if(isset($_POST['sach']))
    {
        $search= $_POST["search"];
        $ser =mysqli_query($con, "SELECT * FROM staff WHERE fname='$search' OR lname ='$search' OR staff_id ='$search' OR email ='$search' ORDER BY staff_id DESC");
        echo (' <br><b><h2>'. $ser->num_rows. ' Records</b></h2>');
            ...
            echo "<table border=0 width='100%' cellspacing=0 >\n";
                    ...
                    echo "  <td><font size=5px><b>Second Name</td>\n";
                    echo "  <td><font size=5px><b>Email</td>\n";
                    echo "  <td><font size=5px><b>Department</td>\n";
                    echo "  <td><font size=5px><b>Extension</td>\n";
    ...
    while($row = mysqli_fetch_assoc($ser))
     {
                        ...
                        echo "  <td contenteditable='false'><b>" . $row["staff_id"] ."</td>\n";                                            
                        ...
                        echo "  <td contenteditable='true'>" . $row["extension"] ."</td>\n";
                        echo "  <td contenteditable='true'>" . $row["trn_date"] ."</td>\n";
                        echo "  <td bgcolor='#566573'><form action='' method='POST'><input type='hidden' name='tempId2' value='".$row["staff_id"]."'/><input type='submit' name='submit-btn' value='Delete' /></form></td>\n";                      
                        echo "  </tr>\n";
...
if (mysqli_num_rows($result) > 0) {
     echo (' <b><h2>'. $result->num_rows. ' Staff Profiles</b></h2>');
                    ...
                    echo "  <td><font size=5px><b>Extension</td>\n";                  
                    echo "  <td width=90px><font size=5px><b>Time</td>\n";
                    echo "  <td width=90px><font size=5px><b>Edit</td>\n";
    ...
    while($row = mysqli_fetch_assoc($result))
     {
                        ...
                        echo "  <td contenteditable='true'>" . $row["extension"] ."</td>\n";
                        echo "  <td contenteditable='true'>" . $row["trn_date"] ."</td>\n";
                        echo "<td>"."<a class='btn btn-primary' href='updatestaff.php?id=".$row['staff_id']."'>"."Edit"."</a>"."</td>\n";
                        echo "  <td bgcolor='#566573'><form action='' method='POST'><input type='hidden' name='tempId2' value='".$row["staff_id"]."'/><input type='submit' name='submit-btn' value='Delete' /></form></td>\n";
                        echo "  </tr>\n";
	}
   if(isset($_POST['submit-btn']))
...
echo" <center><a href='printstaff.php'><input type ='submit' name='print' value='Print'></a><br><br></center>";
mysqli_close($conn);
