...
<form id ="search" method="post">
 ...
 <?php
  if(isset($_POST['sach']))
    {
        $search= $_POST["search"];
        $ser =mysqli_query($con, "SELECT * FROM users WHERE username='$search' OR id ='$search' OR email ='$search' ORDER BY Id DESC");
...
if (mysqli_num_rows($result) > 0) {
     echo (' <b><h2>'. $result->num_rows. ' Security Profiles</b></h2>');
                    ...
                    echo "  <td><font size=5px><b>email</td>\n";
                    echo "  <td width=90px><font size=5px><b>Time</td>\n";
                    echo "  <td><font size=5px><b>Update</td>\n";
    ...
    while($row = mysqli_fetch_assoc($result))
     {
                        ...
                        echo "  <td contenteditable='true'>" . $row["email"] ."</td>\n";
                        echo "  <td contenteditable='true'>" . $row["trn_date"] ."</td>\n";
                        echo "<td>"."<a class='btn btn-primary' href='securityupdate.php?id=".$row['id']."'>"."Edit"."</a>"."</td>\n";
                        echo "  <td bgcolor='#566573'><form action='' method='POST'><input type='hidden' name='tempId2' value='".$row["id"]."'/><input type='submit' name='submit-btn' value='Delete' /></form></td>\n";
                        echo "  </tr>\n";
	}
   if(isset($_POST['submit-btn']))
...
echo" <center><a href='printsecurity.php'><input type ='submit' name='print' value='Print'></a><br><br></center>";
mysqli_close($conn);
